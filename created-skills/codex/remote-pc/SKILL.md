---
description: Drive a Windows or Linux host running agentic-remote-pc over its authenticated MCP runner. Use when the user asks to run commands on a remote PC, drive a coding-agent CLI on a host, inspect health, or validate a local feature/deployment. Triggers include "on my pc", "on the host", "run remotely", "drive the runner".
---
...
4. Agent-to-agent: drive a local agent (e.g. claude_prompt) to implement, then validate with another (e.g. cursor_prompt).
